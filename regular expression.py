...
Nameage = '''Salman is 59 and Hrithik is 51 and Prabhas is 45 and Rajinikanth is 74'''
...
x = re.split(r"\s", txt)  # Split the string using space as the delimiter
print(x)  # Print the list of words


import re
string = 'Twelve:12 Eighty nine:89.'
pattern = r'\d+' # means one or more digit [0-9]
...
num = re.sub(r'\D', "", phone)  # Substitute anything other than digits with an empty string
...
re.sub(r"[A-Za-z]", "*", "ada 1231zxdq #@$@zxfsd312")
...
regex = re.compile("\n") # Use the compiled regular expression pattern to replace newline characters with an empty string.
...
print(len(re.findall(r"\d", randstr)))  # Print the count of digits in the string

# Search for the first digit in the string using regular expression
print(re.search(r"\d{1,2}", randstr))  # Print the first digit found in the string
...
if re.search(r"\d{4}-\d{4}-\w{15}", pan):
...
name = "My name is Baahu, you tell me what is your name"
...
txt = "The rain in Spain" # Define a text string
